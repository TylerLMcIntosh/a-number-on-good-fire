...
name <- "welty_wildfire_fri_splitfrg5"
rasterForVizF <- paste0("gf_data_flatraster_", name, "_california_2010_2020.tif") # The name of the visualization raster in GDrive that you want to use for case study pull out
caseStudyName <- "August Complex"
...
dir_ensure(figsDir)
figsDir2 <- here::here(paste0("figs_", name))
dir_ensure(figsDir2)
figsDir <- figsDir2
...
rxData <- readr::read_csv(here::here('data', 'derived', "twig_rx_state_summary_2010_2020.csv")) #from b02
gfData <- readr::read_csv(here::here('data', 'derived', paste0('gf_data_combined_states_', name, '_2010_2020.csv')))

# Prepped rx spatial data (b02)
rxSpatial <- sf::st_read(here::here(derivedDatDir, 'twig_gee_filtered_ready_for_analysis.gpkg')) |>
  sf::st_transform(sf::st_crs(epsg))

# Prepped gf spatial data (a03)
gfSpatial <- sf::st_read(here::here(derivedDatDir, paste0('merged_goodfire_final_states_', name, '.gpkg'))) |>
  sf::st_transform(sf::st_crs(epsg))
...
gfData <- gfData |>
  dplyr::relocate(unmatchedRegime, .after = totalArea) |>
...
plotLabels2 <- c("Lower Severity Good Forest Wildfire", "Higher Severity Good Forest Wildfire", "Prescribed Burns", "Total Forest Wildfire")
...
allGood <- lowerSevNum + highSevNum
allGood
allGoodOfTot <- allGood / totalWildfireNum * 100
allGoodOfTot

lowerGoodPerYear <- lowerSevNum / 11
lowerGoodPerYear

lowerSevGFRxCompare <- ((lowerSevNum / rxArea) - 1) * 100
lowerSevGFRxCompare
lowerSevGFRxCompare2 <- (lowerSevNum / rxArea)
lowerSevGFRxCompare2
allSevGFRxCompare <- (((lowerSevNum + highSevNum) / rxArea) - 1) * 100
allSevGFRxCompare
allSevGFRxCompare2 <- (lowerSevNum + highSevNum) / rxArea
allSevGFRxCompare2

tooFrequentOfAllBurns <- sum(gfData$tooFrequent) / totalWildfireNum * 100
tooFrequentOfAllBurns
tooFrequentOfGoodFire <- sum(gfData$tooFrequent) / (lowerSevNum + highSevNum) * 100
tooFrequentOfGoodFire

frg5l = sum(gfData$frg5lGood)
frg5h = sum(gfData$frg5hGood)
percFrg5lOfAll5 <- frg5l/(frg5h + frg5l) * 100
percFrg5lOfAll5

# By state
stateSummaryComparison <- stateTable |>
  mutate(comparison = lowerGoodFire / rxBurn)
...
rxLowSevPerc <-  rxLowSev / 
                  rxArea * 100
rxLowSevPerc

rxBroadcastOfLowSev <- (rxSpatial |>
                          dplyr::group_by(FRGDescription, type) |>
...
rxBroadcastOfAllRx <- (rxSpatial |>
                          dplyr::group_by(type) |>
                          dplyr::summarise(tot_area = sum(TOTALACCOMPLISHMENT_FINAL)) |>
                          dplyr::filter(type == "Broadcast Burn") |>
                          dplyr::pull(tot_area)) / 
...
strictLowSev_RxGWF_comparison <- lowerSevNum / rxLowSev
...
#Use only the GF polygons that contain >10% forest pre-burn in the western US & create a combined good fire column

# gfSpatial <- gfSpatial[!sf::st_is_empty(gfSpatial), ]
# sf::sf_use_s2(FALSE)
# gfSpatial <- sf::st_buffer(gfSpatial, dist = 0)
# gfSpatial <- sf::st_make_valid(gfSpatial)
# sf::sf_use_s2(TRUE)
...
if(!file.exists(visRasterLocalPath)) {
  ...
  download_data_from_gdrive_v2(drive_folder = driveFolder,
                               file_name = rasterForVizF,
                               local_path = visRasterLocalPath)
}
...
caseStudyBound <- gfSpatialForestOnly |>
  filter(grepl(pattern = caseStudyName, Listed_F_1, ignore.case = TRUE)) |>
  sf::st_transform(terra::crs(rasterForViz))
...
#case study fire
caseStudyEventFig <-
  tm_shape(caseStudyR, raster.downsample = FALSE) +
  tm_raster(
    # v4: move palette -> col.scale
    col.scale = tm_scale(values = c(bgNeutralCol, gFCol, gfHsCol, totCol))
  ) +
  tm_shape(caseStudyBound) +
  tm_borders(col = boundaryCol, lwd = 1) +
  tm_legend(show = FALSE) +         # v4: turn legend off here
  tm_layout(
    inner.margins = c(0.1, 0.02, 0.02, 0.02),
    frame = FALSE
  ) +
  tm_scalebar(                      # v4: use tm_scale_bar()
    position   = c("right", "bottom"),
    text.size  = 1.2,
    color.dark = boundaryCol,
    breaks = c(0, 10, 20, 30, 40)
    # omit width to avoid the sbW bug; v4 will pick a sensible width
  )

# safer save (with basic error handling)
ok <- tryCatch({
  tmap_save(
    caseStudyEventFig,
    filename = here::here(figsDir, "case_study_fig_event.png")
  )
  TRUE
}, error = function(e) {
  message("tmap_save failed: ", conditionMessage(e))
  FALSE
})
